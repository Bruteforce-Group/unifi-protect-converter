...
class OrderedLinq extends EnumerableImpl {
    ...
    ThenBy(keySelect, equal) {
        if (!keySelect && !equal)
            return this;
        const compare = keySelect ? equal ? (a, b) => equal(keySelect(a), keySelect(b))
            : (a, b) => Constant.defCompare(keySelect(a), keySelect(b))
            : equal;
        const superEqual = this.comparer;
        const comparer = (!superEqual) ? compare
            : (a, b) => superEqual(a, b) || (this.reversed ? -compare(a, b) : compare(a, b));
        return new OrderedLinq(this._target, this._factory, undefined, comparer, this.reversed);
    }
